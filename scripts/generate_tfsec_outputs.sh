...
RESULT_DIR="$ROOT_DIR/results/tfsec"
...
echo ""
echo "Running TFSEC on the following directories ... "
if [[ ${#directories_to_process[@]} -gt 0 ]]; then # Check if the array is not empty
  ...
  for dir in "${directories_to_process[@]}"; do
    ...
    echo "  Processing: $dir"
    tfsec "$OUTPUT_DIR/$dir" -f text > $RESULT_DIR/$dir/tfsec_output.txt
    tfsec "$OUTPUT_DIR/$dir" -f json > $RESULT_DIR/$dir/tfsec_output.json
  done
else
